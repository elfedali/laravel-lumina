                        ...
                        <header class="d-flex justify-content-between align-items-start">
                            ...
                            <div>
                                <a href="{{ route('users.create') }}" class="btn btn-primary">
                                    <svg viewBox="0 0 24 24"><path fill="currentColor" d="M11 13H5v-2h6V5h2v6h6v2h-6v6h-2z"/></svg>
                                ...
                                <tbody>
                                    @foreach ($users as $client)
                                        <tr>
                                            ...
                                            <td>{{ $client->shop_name ?? 'N/A' }}</td>
                                            <td>{{ $client->email ?? 'N/A' }}</td>
                                            <td>{{ $client->telephone ?? 'N/A' }}</td>
                                            <td>{{ $client->created_at->diffForHumans() }}</td>
                                            <td class="text-end">
                                                <div class="btn-group">
                                                    <a href="{{ route('users.edit', $client->id) }}" class="btn btn-sm btn-light">
                                                        <i class="fa-solid fa-pen"></i> {{ __('Modifier') }}
                                                    </a>
                                                    <button class="btn btn-sm btn-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $client->id }}').submit();">
                                                        <i class="fa-solid fa-trash"></i> {{ __('Supprimer') }}
                                                    </button>
                                                </div>
                                                <form action="{{ route('users.destroy', $client->id) }}" method="POST" id="delete-form-{{ $client->id }}" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
